...
export function Transcript({
  ...
  useEffect(() => {
    ...
    const words = data?.monologues ? data.monologues.reduce((acc:any, block:Monologue) => {
    ...
    const onTimeUpdate = () => {
      const wordNodes = document.getElementsByClassName('text') as HTMLCollectionOf<HTMLElement>;
      const hl = document.getElementById("highlighter")
      const activeIndex = words.findIndex((word:any) => {
        return word.ts >= playerRef.current.currentTime - 0.25 && playerRef.current.currentTime <= word.end_ts
      })
  ...
  return (
    <div className="relative">
      <h2 className="font-bold text-2xl mb-10">Transcript <span>(auto-generated)</span></h2>
      <span id="highlighter" className="absolute pl-1 pt-0.5 rounded-sm text-xl transition-all duration-75 w-12 text-black bg-highlight-600 z-10 hidden"/>
      <div className="text-xl">
        {data.monologues.map((block:any, id:number) => {
          return <div key={`s_${block.speaker}_id_${id}`} className="mb-8 leading-normal">
            {/*<div className="w-48"><strong>Speaker {block.speaker}:</strong></div>*/}
            <div 
              className="mb-2" 
              onClick={() => handleCueClick(block.elements[0].ts)}>
                <strong className="cursor-pointer hover:text-highlight-600">Speaker {block.speaker} [{ formatDuration(block.elements[0].ts)}]</strong>
            </div>
            <p className="">
              {block.elements.map((el:any, id:number) => {
                return <span 
                  ...
                  onClick={() => handleCueClick(el.ts)}>{el.value}
                </span>
              })}
            </p>
          </div>
